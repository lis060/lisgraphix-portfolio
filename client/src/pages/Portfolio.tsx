            ...
            <Reveal key={project.id} delay={i * 0.08}>
              <div className="group rounded-2xl overflow-hidden border border-white/10 bg-charcoal/60 hover:border-gold/40 transition-all duration-300 hover:-translate-y-1 h-full">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-ink/90 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-5">
                    <div className="flex gap-3 flex-wrap">
                      ...
                      {project.demoUrl && (
                        <Link
                          to={project.demoUrl}
                          className="px-3 py-1.5 rounded-lg bg-white/10 text-white text-xs font-semibold flex items-center gap-1 hover:bg-white/20 transition-colors"
                        >
                ...
                <div className="p-5">
                  ...
                  {(project.demoUrl || project.caseStudySlug) && (
                    <div className="flex gap-2 mt-4 pt-4 border-t border-white/5 flex-wrap">
                      {project.demoUrl && (
                        <Link
                          to={project.demoUrl}
                          className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-gold text-ink text-xs font-bold hover:bg-gold/90 transition-colors"
                        >
                      ...
                      {project.caseStudySlug && (
                        <Link
                          to={`/case-studies/${project.caseStudySlug}`}
                          className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg border border-white/10 text-white/70 text-xs font-medium hover:border-gold/40 hover:text-white transition-colors"
                        >
                          Case Study
                        </Link>
                      )}
